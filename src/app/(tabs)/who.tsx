...
import { useInvitationStore, initializeDemoInvitations } from '@/lib/state/invitation-store';
...
import { InvitationCreationModal } from '@/components/InvitationCreationModal';
...
export default function WhoScreen() {
  ...
  const getInvitationsByWorkspace = useInvitationStore(s => s.getInvitationsByWorkspace);
  ...
  // Get invitations for current workspace
  const workspaceInvitations = useMemo(() => {
    if (!currentWorkspace?.id) return [];
    return getInvitationsByWorkspace(currentWorkspace.id);
  }, [currentWorkspace?.id, invitations, getInvitationsByWorkspace]);

  const pendingInvitations = useMemo(() =>
    workspaceInvitations.filter(i => i.status === 'pending' || i.status === 'counter_offered'),
    [workspaceInvitations]
  );

  // Initialize demo invitations on mount
  useEffect(() => {
    if (invitations.length === 0) {
      initializeDemoInvitations();
    }
  }, []);
  ...
  // Approval/Rejection commentary modal
  ...
  const [commentaryRequestId, setCommentaryRequestId] = useState<string | null>(null);
  const [commentaryRequestName, setCommentaryRequestName] = useState('');
  ...
  return (
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      >
        ...
        {activeTab === 'team' && (
          <View>
                ...
                {pendingRequests.slice(0, 2).map(req => (
                  <View key={req.id} className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 mb-2">
                    ...
                    <View className="flex-row gap-2 mt-2">
                      <Pressable
                        onPress={() => {
                          setCommentaryRequestId(req.id);
                          setCommentaryRequestName(req.candidateName);
                          setCommentaryAction('approve');
            ...
            {/* Invitations Section (Founder only) */}
            {isFounder && (
              <View className="mb-6">
                <View className="flex-row items-center justify-between mb-3">
                  <View className="flex-row items-center gap-2">
                    ...
                    {pendingInvitations.length > 0 && (
                      <View className="bg-purple-500 px-2 py-0.5 rounded-full">
                  ...
                  <Pressable
                    ...
                    className="flex-row items-center gap-1 bg-purple-500 px-3 py-1.5 rounded-lg active:opacity-80"
                  >
                    <Plus size={14} color="white" />
                ...
                {workspaceInvitations.length === 0 ? (
                  <View className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6 items-center">
                    <FileText size={32} color="#94a3b8" />
                    <Text className="text-slate-500 dark:text-slate-400 text-center mt-3 text-sm">
                      No invitations yet
                    </Text>
                    <Text className="text-slate-400 dark:text-slate-500 text-center mt-1 text-xs">
                      Send invitations to build your team
                    </Text>
                  </View>
                ) : (
                  <View className="gap-3">
                    {workspaceInvitations.slice(0, 3).map(invitation => (
                      <InvitationListItem
                        key={invitation.id}
                        invitation={invitation}
                      />
                    ))}
                    {workspaceInvitations.length > 3 && (
                      <Pressable className="py-2 items-center">
                        <Text className="text-purple-500 text-sm font-medium">
                          View all {workspaceInvitations.length} invitations
                        </Text>
                      </Pressable>
                    )}
                  </View>
      ...
      {/* Commentary Modal for Approval/Rejection */}
      <CommentaryInput
        visible={showCommentaryModal}
        action={commentaryAction}
        requestType="hiring request"
        requestName={commentaryRequestName}
        onSubmit={(commentary) => {
          if (commentaryRequestId) {
            if (commentaryAction === 'approve') {
              approveRequest(commentaryRequestId, currentUser?.id || 'founder', commentary || undefined);
            } else {
              rejectRequest(commentaryRequestId, currentUser?.id || 'founder', commentary);
            }
          }
          setShowCommentaryModal(false);
          setCommentaryRequestId(null);
        }}
        onCancel={() => {
          setShowCommentaryModal(false);
          setCommentaryRequestId(null);
