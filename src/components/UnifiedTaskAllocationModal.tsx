...
import {
  ...
  TrendingUp, AlertCircle, Archive, Target, Play, Pause, Calendar
} from 'lucide-react-native';
import { RescheduleDateButtons } from './RescheduleDateButtons';
import { EmailNotificationPrompt } from './EmailNotificationPrompt';
...
export function UnifiedTaskAllocationModal({ visible, onClose, workPlan }: Props) {
  ...
  // Email notification prompt state
  const [showEmailPrompt, setShowEmailPrompt] = useState(false);
  const [pendingEmailAssignments, setPendingEmailAssignments] = useState<Array<{
  ...
  const handleSave = useCallback(() => {
    ...
    // Find newly assigned members (not in previous allocations)
    const previousMemberIds = new Set(workPlan.allocations?.map(a => a.memberId) || []);
    const newlyAssignedMemberIds = allocationArray
      .map(a => a.memberId)
      .filter(id => !previousMemberIds.has(id));
    ...
    if (newlyAssignedMemberIds.length > 0 && workPlan.workspaceId) {
      newlyAssignedMemberIds.forEach(memberId => {
        const notification = notificationHelpers.newAssignment(
          workPlan.workspaceId,
          workPlan.title
        );
        addNotification({
          ...notification,
          actionRoute: '/what',
          actionData: { taskId: workPlan.id, memberId },
        });
      });

      // Prepare email assignments for prompt
      const emailAssignments = newlyAssignedMemberIds
        .map(memberId => {
          const member = members.find(m => m.id === memberId);
          const allocation = allocationArray.find(a => a.memberId === memberId);
          if (member) {
            return {
              member,
              tuPerWeek: allocation?.squaresPerWeek || 0,
            };
          }
          return null;
        })
        .filter((a): a is { member: OrganizationMember; tuPerWeek: number } => a !== null);

      // Show email prompt if there are newly assigned members and preference is enabled
      if (emailAssignments.length > 0 && emailPromptEnabled) {
        setPendingEmailAssignments(emailAssignments);
        setShowEmailPrompt(true);
        return; // Don't close yet, wait for email prompt
      }
    }

    onClose();
  }, [workPlan, allocations, selectedAITool, totalTUs, calculations, members, updateWorkPlan, getCostPerSquare, getAITool, onClose, addNotification, emailPromptEnabled]);
        ...
        <Pressable onPress={(e) => e.stopPropagation()} style={{ maxHeight: '92%' }}>
          <View className="bg-gray-50 dark:bg-slate-950 rounded-t-3xl" style={{ height: '100%' }}>
            ...
            >
              ...
              >
                <Pressable
                  onPress={() => setExpandedSection(expandedSection === 'quick' ? null : 'quick')}
                ...
                >
                  ...
                  <ChevronRight
                    size={20}
                    color="#6b7280"
                    style={{ transform: [{ rotate: expandedSection === 'quick' ? '90deg' : '0deg' }] }}
                  />
                </Pressable>
                ...
                {expandedSection === 'quick' && (
                  <View className="mt-4">
                    {/* Status Change */}
                    <View className="mb-4">
                      <Text className="text-gray-500 dark:text-slate-400 text-xs font-semibold mb-2">
                        QUICK STATUS CHANGE
                      </Text>
                      <View className="flex-row gap-2">
                        <Pressable
                      ...
                      <Text className="text-gray-400 dark:text-slate-500 text-[10px] text-center mt-2 italic">
                    ...
                    <View className="mb-4">
                      ...
                      <View className="flex-row items-center gap-2">
                        <Pressable
                          onPress={() => updateWorkPlan(workPlan.id, { progress: Math.max(0, workPlan.progress - 25) })}
                          className="w-8 h-8 bg-gray-100 dark:bg-slate-800 rounded-lg items-center justify-center"
                        >
                        ...
                        <Pressable
                          onPress={() => updateWorkPlan(workPlan.id, { progress: Math.min(100, workPlan.progress + 25) })}
                          className="w-8 h-8 bg-gray-100 dark:bg-slate-800 rounded-lg items-center justify-center"
                        >
                    ...
                    <View>
                      ...
                      <RescheduleDateButtons
                        currentDate={workPlan.dueDate}
                        onReschedule={(newDate) => updateWorkPlan(workPlan.id, { dueDate: newDate })}
                        showReset={false}
                        compact={false}
